...
export default function EmployeeFilePreview() {
  const { rows, fileName, setFileFromText, clearFile } = useEmployeeImport();
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <CardDescription className="text-xs text-zinc-500">
                  {fileName
                    ? `Source: ${fileName}`
                    : 'Upload a CSV to preview mapped rows before commit.'}
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="p-0">
          {rows.length === 0 ? (
            <div className="p-8 text-center text-xs text-zinc-500">
              No file loaded. Download the template, fill employee rows, then
              select the CSV file to review.
            </div>
          ) : (
                    ...
                    const name =
                      row.data
                        ...
                        : [row.raw.firstName, row.raw.lastName]
                            ...
                            .join(' ') || '—';
                    const department =
                      row.data?.department ?? row.raw.department ?? '—';
                    const designation =
                      row.data?.designation ?? row.raw.designation ?? '—';
                    const dateJoined =
                      row.data?.joiningDate ?? row.raw.joiningDate ?? '—';
                    const employeeId =
