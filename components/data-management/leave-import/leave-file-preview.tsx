...
import { useLeaveImport } from './leave-import-context';
...
export default function LeaveFilePreview() {
  const { rows, fileName, setFileFromText, clearFile } = useLeaveImport();
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <CardDescription className="text-xs text-zinc-500">
                  {fileName
                    ? `Source: ${fileName}`
                    : 'Upload a CSV to preview mapped absence rows before commit.'}
                </CardDescription>
              </div>
            </div>
        ...
        <CardContent className="p-0">
          {rows.length === 0 ? (
            <div className="p-8 text-center text-xs text-zinc-500">
              No file loaded. Download the template, fill leave rows (YYYY-MM-DD
              dates), then select the CSV. Blank trailing rows are ignored.
            </div>
          ) : (
              ...
              <Table className="text-xs text-left w-full min-w-[700px]">
                ...
                <TableBody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                  {rows.map((row) => {
                    ...
                    const leaveType =
                      row.data?.leaveType ?? row.raw.leaveType ?? '—';
                    const startDate =
                      row.data?.startDate ?? row.raw.startDate ?? '—';
                    const endDate =
                      row.data?.endDate ?? row.raw.endDate ?? '—';
                    const reason =
                      row.data?.reason ?? row.raw.reason ?? '—';
                    const days =
                      row.previewTotalDays != null
                        ? row.previewTotalDays
                        : row.raw.totalDays || '—';
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="px-5 py-4 font-medium text-zinc-600 dark:text-zinc-300">
                          <Badge
                            variant="outline"
                            className="text-[10px] font-semibold bg-zinc-55 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-350 border-none rounded py-0.5 px-2"
                          >
                            {leaveType}
                          </Badge>
                        </TableCell>
                        ...
                        <TableCell className="px-5 py-4 text-zinc-500 dark:text-zinc-400 max-w-[160px] truncate">
                          {reason}
                        </TableCell>
