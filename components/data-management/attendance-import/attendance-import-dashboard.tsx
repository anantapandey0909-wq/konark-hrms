...
import { Clock, Info, ShieldAlert, Loader2 } from 'lucide-react';
...
import {
  ...
  useAttendanceImport,
} from './attendance-import-context';
...
function AttendanceImportDashboardInner() {
      ...
      <div className="flex flex-col gap-1.5 md:flex-row md:items-center md:justify-between pb-4 border-b border-zinc-150 dark:border-zinc-800">
        <div className="space-y-1">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-2xl leading-relaxed">
            Import raw terminal logs, biometric check-ins, and override shifts.
            Standardize columns to verify calculations, worked hours, and status
            parameters prior to core payroll integration.
          </p>
        </div>
      </div>
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <p className="leading-relaxed text-[11px] text-indigo-700/90 dark:text-indigo-400/90">
                Worked hours are calculated dynamically based on local check-in
                and check-out stamps. Overtime hours are automatically compiled
                based on organizational guidelines (standard day = 8 hours).
              </p>
            </div>
          </motion.div>
        </div>
          ...
          <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex gap-2 text-xs text-zinc-500 dark:text-zinc-450 items-start">
              ...
              <div className="leading-normal text-[11px] space-y-1">
                <p>
                  Valid biometric parameters. Rows with warning markers can be
                  bypassed during the synchronization write operation.
                </p>
                {importResult && (
            ...
            >
              ...
              {isImporting
                ? "Committing…"
                : validCount > 0
                  ? `Commit Attendance Records (${validCount})`
...
export default function AttendanceImportDashboard() {
  return (
    <AttendanceImportProvider>
      <AttendanceImportDashboardInner />
    </AttendanceImportProvider>
  );
}
