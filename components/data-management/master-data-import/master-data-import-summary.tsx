...
import type { MasterDataType } from './master-data-selector';
...
interface MasterDataImportSummaryProps {
  selectedType?: MasterDataType;
}

export default function MasterDataImportSummary({
  selectedType = 'departments',
}: MasterDataImportSummaryProps) {
  const {
    totalCount,
    validCount,
    invalidCount,
  ...
  const metrics = isDepartments
    ? [
        { label: 'Total Records', count: totalCount, icon: FileSpreadsheet },
        { label: 'Ready to Import', count: validCount, icon: CheckCircle },
        {
          label: 'System Warnings',
          count: blankRowsSkipped + (importResult?.failedCount ?? 0),
          icon: AlertTriangle,
        },
        { label: 'Critical Failures', count: invalidCount, icon: AlertCircle },
      ]
    : [
        { label: 'Total Records', count: 0, icon: FileSpreadsheet },
        { label: 'Ready to Import', count: 0, icon: CheckCircle },
        { label: 'System Warnings', count: 0, icon: AlertTriangle },
        { label: 'Critical Failures', count: 0, icon: AlertCircle },
      ];
